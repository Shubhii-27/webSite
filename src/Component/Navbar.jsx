...
const Navbar = () => {
  ...
  return (
    <>
      {/* Navbar */}
      <nav className="fixed top-0 left-0 w-full z-50 bg-gray-800 text-white shadow-md h-20">
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-full">
          ...
          <div className="flex items-center gap-3">
            <img
              src={img}
              alt="Website Logo"
              className="w-20 h-20 rounded-full border-blue-500"
            />
          </div>
        ...
        {/* Mobile Menu */}
        <div
          className={`md:hidden right-0 bg-gray-800 px-6 overflow-hidden transition-all duration-300 text-2xl text-center ${isOpen ? "max-h-96 py-4" : "max-h-0"
            }`}
        >
          <div className="flex flex-col space-y-4">
            ...
            <NavLink onClick={() => setIsOpen(false)} to="/login" className="block text-center bg-blue-600 py-2 rounded-lg hover:bg-blue-700 transition">Login</NavLink>
          </div>
        </div>
      </nav>
    </>
